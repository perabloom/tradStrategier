...
class MARKET_MAKE:
  ...
  def handle_data(self, data):
    ...
    log().debug("MARKET_MAKE" +  str(data))
    data = data[0]
    start = time.perf_counter()
    orders = orderUtil.getOrdersIdsWithStateForSymbol('open', data['symbol']) # send symbol
    end = time.perf_counter()
    positions = position_util.getPositions(data['symbol']) # Send symbol
    ...
    if (data['type'] == 'quote' or data['type'] == 'timesale'):
      ...
      if (my_bid >= my_ask):
        log().debug("My Bid >= My Ask, cancelling any orders and/or close positions if any")
        if (order_exists):
          tradeUtil.cancelOrder(orders[0]['id'])
        if (position_exists):
          tradeUtil.closePositions(positions)
        return
      if (position_exists == False):
        ...
        if (order_exists == True):
          ...
          if (side == 'buy'):
            ...
            if (price == my_bid):
              log().debug("NO ADJUSTMENT OF BID NEEDED as my_bid equals my expected theo bid")
              pass
            else:
              # cancel the order and resend a new one
              # It will fail if it's filled, in which case handle the exception and return
              try:
                log().debug("Cancelling existing bid for order -" + str(orders['id']))
            ...
            log().debug("ERROR - There should not be any sell order if NO POSITION exists")
            raise Exception("No position exists")
        else:
          # No position or order, send a buy limit order
          log().debug('NO ORDER EXISTS - Sending an Order')
          order_id = tradeUtil.sendOrder('equity', data['symbol'], 'buy', QUANTITY, my_bid, ORDER_TYPE, DURATION, data['symbol'])
      else:
        # if position exists
        ...
        if (order_exists == True):
            ...
            if (price == my_ask):
              ...
              # It will fail if it's filled, in which case handle the exception and return
              try:
                log().debug("Cancelling existing ask for order -" + str(orders['id']))
                tradeUtil.cancelOrder(orders['id'])
              except Exception as e:
                log().debug("ERROR : Failed to cancel order " + str(orders['id']) + " with exception : " + str(e))
                return
          ...
          log().debug('POSITION EXISTS BUT NO SELL ORDER EXIST - Sending an SELL Order')
          order_id = tradeUtil.sendOrder('equity', data['symbol'], 'sell', QUANTITY, my_ask, ORDER_TYPE, DURATION, data['symbol'])
